...
class CollabFilteringMetrics:
    ...
    def __init__(self, filename, size = 0, tests = 0):
        ...
        self.users, self.skills = create_users(self.dataset.interactions)
    ...
    def top_n_questions(self, anchor, search_size):
        ...
        closest_10_percent = max(len(sorted_indexes) // 20 , 10)
        ...
        questions_to_choose = convert_distances(closest_users_dists, search_size)
        ...
        break_loop = 0
        while remaining > 0:
            id = self.dataset.problem_ids.sample(1).item()
            if not id in top_n:
                top_n.append(id)
                remaining -= 1
            else:
    ...
    def rank_questions(self, ids, anchor):
        ...
        dists_from_users = dists_from_users[dists_from_users > 0]
        sorted_indexes = np.argsort(dists_from_users)
        closest_5_percent = max(len(sorted_indexes) // 100 , 10)
        best_idxs = sorted_indexes[:closest_5_percent]
        closest_users_dists = dists_from_users[best_idxs]
        closest_users = [self.users[i] for i in best_idxs]
        # t_1 = time.time()
...
class User:
    ...
    def get_correct_ids(self, questions):
        df = self.is_completed.loc[questions]
...
def create_users(dataset):
    ...
    all_questions = dataset.problem_id.unique()
    ...
    for user in tqdm(all_users):
        ...
        new_user = User(user_scores, all_skills, interactions)
